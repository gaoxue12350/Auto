...
class Tag(BaseApi):
    ...
    # 获取标签
    def list(self):
        data_all=Template(self.data).safe_substitute(token=self.token)
        data=yaml.safe_load(data_all)['list']
        r=self.send(data)
    ...
    def update(self,id,tag_name):
        data_all:str=Template(self.data).safe_substitute(token=self.token,id=id,tag_name=tag_name)
        data:dict=yaml.safe_load(data_all)['update']
        r=self.send(data)
        return r

    def update_and_detect(self,id,tag_name):
        r=self.update(id,tag_name)
        if r.json()['errcode']==40068:
            if not self.is_tag_id_exist(id):
                self.add_and_detect('group123',[{"name": 'tag_name'}])
                id=self.find_tag_id_by_name('tag_name')
            r=self.update(id,tag_name)
        return r
    ...
    def is_group_id_exist(self,group_id):
        r=self.list()
        group_ids:list=jsonpath(r.json(),'$..group_id')
    ...
    def add(self,group_name,tag,**kwargs):
        data:str=Template(self.data).safe_substitute(token=self.token,group_name=group_name,tag=tag)
        data:dict=yaml.safe_load(data)['add']
        r=self.send(data)
        # print(json.dumps(r.json(), ensure_ascii=False,indent=2))
        return r
    ...
    def generate_unique_tag_name(self,tag_name):
        tag_name1=tag_name + str(datetime.datetime.now().strftime('%Y%d%H%M%S'))
        return tag_name1

    def delete_tag(self,tag_id):
        data_all=Template(self.data).safe_substitute(token=self.token,tag_id=tag_id)
        data=yaml.safe_load(data_all)['delete_tag']
        r=self.send(data)
        # print(json.dumps(r.json(),ensure_ascii=False, indent=2))
        return r
    ...
    def delete_group(self, group_id):
        data_all=Template(self.data).safe_substitute(token=self.token,group_id=group_id)
        data=yaml.safe_load(data_all)['delete_group']
        r = self.send(data)
        # print(json.dumps(r.json(),ensure_ascii=False,indent=2))
        return r
    ...
    def is_tag_id_exist(self,tag_id):
        tag_id_total:list=jsonpath(self.list().json(),"$..id")
        if tag_id in tag_id_total:
            return tag_id
        # for group in self.list().json()['tag_group']:
